...
def detail(request):
    ...
    if request.method == "GET":
        ...
        if not bool(comment):
            return JsonResponse({"error message": "no comments on the requested post."},status=HTTPStatus.OK)
...
def post(request):
    if request.method == "POST":
        ...
        return JsonResponse({"message": "comment was created successfuly!"}, status=HTTPStatus.CREATED)
    return JsonResponse({"error message": "method is not supported."}, status=HTTPStatus.NOT_FOUND)
...
def update(request):
    if request.method == "PUT":
        ...
        try:
            ...
            comment.save(0)
            return JsonResponse({"message": "comment body is updated successfully."}, status=HTTPStatus.OK)
        except:
            return JsonResponse({"error message": "comment does not exist."}, status=HTTPStatus.NOT_FOUND)
    return JsonResponse({"error message": "method is not supported."}, status=HTTPStatus.NOT_FOUND)
...
def delete(request):
    if request.method == "DELETE":
        ...
        try:
            comment = Comment.objects.get(pk=comment_id)
            comment.delete()
            return JsonResponse({"message": "comment was deleted successfully."}, status=HTTPStatus.OK)
        except:
            return JsonResponse({"error message": "comment does not exist."}, status=HTTPStatus.NOT_FOUND)
    return JsonResponse({"error message": "method is not supported."}, status=HTTPStatus.NOT_FOUND)
